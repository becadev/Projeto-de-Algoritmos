...
class linked_list {
    ...
    public:
        ...
        void push_front(int value) { // Adiciona um elemento no ``início'' do vetor
            int_node *new_node = new int_node; 
            new_node-> value = value;
            new_node-> next  = this->head; // Aponta pro primeiro elemento da lista, head = [[prev], [v1] , [next]]
            new_node-> prev  = nullptr;    // 
            if(this->head == nullptr)      // Caso a lista esteja vazia a ponta vai apontar para o endereço do proprio nó,  
                this->tail = new_node;
            else
